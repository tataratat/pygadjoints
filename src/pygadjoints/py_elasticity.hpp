...
class LinearElasticityProblem {
   ...
   public:
    ...
    void Init(const int numRefine) {
        ...
        basis_function_ptr->setup(boundary_conditions, dirichlet::l2Projection,
                                  0);
    ...
    void Assemble() {
        ...
        auto bilin_lambda = lame_lambda_ *
                            idiv(basis_function, geometric_mapping) *
        ...
        system_matrix = std::make_shared<const gsSparseMatrix<>>(
    ...
    double ComputeObjectiveFunction() {
      ...
      real_t volume_value =
          expr_evaluator.integral(meas(*geometry_expression_ptr));
      // Compute volume of domain
      return volume_value;
    }
    ...
    py::array_t<double> ComputeVolumeDerivativeToCTPS() {
        ...
        double *derivative_ptr =
            static_cast<double *>(derivative.request().ptr);
    ...
    py::array_t<double> ComputeObjectiveFunctionDerivativeWrtCTPS() {
        ...
        auto BL_lambda_2 =
            idiv(basis_function, geometric_mapping);  // validated
        auto BL_lambda =
        ...
        auto BL_lambda_1_dx = frobenius(
            aux_expr,
            ijac(solution_expression, geometric_mapping));  // validated
        auto BL_lambda_2_dx =
        ...
        auto BL_lambda_dx =
            ...
            lame_lambda_ * BL_lambda_2 * BL_lambda_1_dx *
                meas_expr;  // validated
        ...
        auto BL_mu2_dx1 = lame_mu_ * frobenius(BL_mu2_2_dx, BL_mu2_1) *
                          meas_expr;  // validated
    ...
    py::tuple GetParameterSensitivities(
        ...
        for (int patch_support{}; patch_support < patch_supports.rows();
            ...
            for (index_t k_dim = 0; k_dim != dimensionality_; k_dim++) {
                for (size_t l_dof = 0;
                     l_dof != dof_mapper_ptr->patchSize(j_patch, k_dim);
                     l_dof++) {
                    if (dof_mapper_ptr->is_free(l_dof, j_patch, k_dim)) {
                        const int global_id =
                            dof_mapper_ptr->index(l_dof, j_patch, k_dim);
                        ...
                        if (counter == n_entries) {
                            throw std::runtime_error(
                                "Underestimated number of entries");
                        }
                    }
                }
            }
    ...
    void UpdateGeometry(const std::string &fname,
        ...
        for (size_t patch_id{}; patch_id < mp_new.nPatches(); patch_id++) {
            ...
            for (int i_coef = 0;
                 i_coef != mp_pde.patch(patch_id).coefs().size(); i_coef++) {
